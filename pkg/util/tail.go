...
func UpdateConfig(confs []settings.Collect) (err error) {
...
func createNewTask(conf settings.Collect) {
	...
	if errTail != nil {
		fmt.Println("collect filename[%s] failed, err:%v", conf.LogPath, errTail)
		return
	}
...
func InitTail(conf []settings.Collect, chanSize int) (err error) {

	tailObjMgr = &TailObjMgr{
		msgChan: make(chan *TextMsg, chanSize),
	}
	if len(conf) == 0 {
		fmt.Println("invalid config for log collect, conf:%v", conf)
		return
	}
	for _, v := range conf {
		createNewTask(v)
